...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // CORS ayarları (Frontend'den rahatça tetiklenebilmesi için)
  res.setHeader('Access-Control-Allow-Credentials', 'true');
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET,OPTIONS,PATCH,DELETE,POST,PUT');
  res.setHeader(
    'Access-Control-Allow-Headers',
    'X-CSRerf-Token, X-Requested-With, Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-Version'
  );
  ...
  try {
    ...
    try {
      ...
      console.error('Fiyat hatası:', priceErr);
    }

    const rsi = 54.20;

    // 3. Şık Bülten Metni (Resmin altına gelecek olan alt yazı - Caption)
    ...
    reportMessage += `━━━━━━━━━━━━━━━━━\n\n`;
    reportMessage += `🔮 <b>Yapay Zeka Görüşü:</b> Market yapısı kararlı duruşunu koruyor. Grafikteki hacim girişleri yukarı yönlü ivmeyi destekliyor.\n\n`;
    ...
    // 4. TradingView Canlı Grafik Resim Linki (Doğrudan PNG Çıktısı)
    ...
    const telegramRes = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`, {
      ...
      body: JSON.stringify({
        ...
        photo: chartImageUrl,      // Ekran görüntüsü resmi buraya oturuyor
        caption: reportMessage.trim(), // Bilgiler de tam altına yapışıyor
        parse_mode: 'HTML'
      }),
    });
